...
import { ArrowLeft, AlertCircle, Shuffle, Bot, Loader2 } from 'lucide-react'
...
export default function NewSessionPage() {
  ...
  const [interviewLanguage, setInterviewLanguage] = useState<InterviewLanguage>('english')
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-white rounded-xl border p-6 space-y-3">
          ...
          <FileUploadZone label="Upload CV (PDF)" fileType="cv"
            onTextExtracted={(text, path) => { setCvText(text); setCvFilePath(path) }}
            disabled={loading} />
          {(!cvFilePath || cvNeedsPaste) && (
            <div className="pt-1 space-y-1.5">
              ...
              <Textarea placeholder="Paste your CV content here..." value={cvText}
                onChange={(e) => setCvText(e.target.value)} className="min-h-[100px]" disabled={loading} />
            </div>
          )}
        </div>
        ...
        <div className="bg-white rounded-xl border p-6 space-y-3">
          ...
          <Tabs defaultValue="upload">
            ...
            <TabsContent value="upload" className="space-y-3">
              <FileUploadZone label="Upload Job Description (PDF)" fileType="jd"
                onTextExtracted={(text, path) => { setJdText(text); setJdFilePath(path) }}
                disabled={loading} />
              {jdNeedsPaste && (
                <div className="space-y-1.5">
                  ...
                  <Textarea placeholder="Paste the job description here..." value={jdText}
                    onChange={(e) => setJdText(e.target.value)} className="min-h-[120px]" disabled={loading} />
                </div>
              )}
            </TabsContent>
            <TabsContent value="paste">
              <Textarea placeholder="Paste the job description here..." value={jdText}
                onChange={(e) => setJdText(e.target.value)} className="min-h-[160px]" disabled={loading} />
            </TabsContent>
          </Tabs>
        </div>
        ...
        <div className="bg-white rounded-xl border p-6 space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Input placeholder="e.g. Shopee Backend Engineer – April 2026" value={sessionName}
              onChange={(e) => setSessionName(e.target.value)} disabled={loading} maxLength={200} />
          </div>

          <div className="space-y-2">
            <Label>Additional Context</Label>
            <Textarea placeholder="e.g. Target company is Shopee. I'm weak on system design. Interview is in 2 weeks."
              value={extraInfo} onChange={(e) => setExtraInfo(e.target.value)}
              disabled={loading} maxLength={500} className="min-h-[80px]" />
            <p className="text-xs text-muted-foreground text-right">{extraInfo.length}/500</p>
          </div>

          <div className="space-y-2">
            <Label className="flex items-center gap-1.5"><Bot className="w-3.5 h-3.5" /> AI Model</Label>
            <ModelPicker value={aiModel} onChange={setAiModel} />
          </div>

          <div className="space-y-2">
            <Label>Interview Language</Label>
            <LanguagePicker value={interviewLanguage} onChange={setInterviewLanguage} />
          </div>

          <button type="button" disabled={loading} onClick={() => setShuffleQuestions((v) => !v)}
            className={`w-full flex items-center justify-between rounded-lg border px-4 py-3 transition-colors ${
              shuffleQuestions ? 'border-primary bg-primary/5' : 'border-border hover:border-muted-foreground/40'
            }`}>
            <div className="flex items-center gap-2.5">
              <Shuffle className={`w-4 h-4 ${shuffleQuestions ? 'text-primary' : 'text-muted-foreground'}`} />
              <div className="text-left">
                <p className={`text-sm font-medium ${shuffleQuestions ? 'text-primary' : ''}`}>Mix question order</p>
                <p className="text-xs text-muted-foreground">Randomize focus areas so questions come in unpredictable order</p>
              </div>
            </div>
            <div className={`w-9 h-5 rounded-full transition-colors flex items-center px-0.5 ${shuffleQuestions ? 'bg-primary' : 'bg-muted'}`}>
              <div className={`w-4 h-4 rounded-full bg-white shadow transition-transform ${shuffleQuestions ? 'translate-x-4' : 'translate-x-0'}`} />
            </div>
          </button>
        </div>
        ...
        {!canSubmit && (
          <p className="text-sm text-muted-foreground text-center">
            {!cvReady && !jdReady ? 'Upload or paste both your CV and job description to continue'
              : !cvReady ? 'Add your CV to continue' : 'Add the job description to continue'}
          </p>
        )}
